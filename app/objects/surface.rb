...
class Surface < Container
  ...
  def initialize(*args, capacity: nil) #This is a hashed argument - called with Surface.new(capacity = 1) if I'm not mistaken. Used to be: def initialize(capacity = nil, *args)
    super(*args)
  ...
  def look_on(event)
    event[:player].output("#{self.name} has:\n" << @inventory.show)
  end
end
